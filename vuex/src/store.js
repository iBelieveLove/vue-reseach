...
export class Store {
  ...
  constructor (options = {}) {
    ...
    */
    ...
    this._committing = false // 是否在进行提交状态标识, 通过监听此标识判断是否在外部修改vuex状态
...
/* 通过vm重设store，新建Vue对象使用Vue内部的响应式实现监听state以及computed */
function resetStoreVM(store, state, hot) {
  ...
  /*  这里new了一个Vue对象，运用Vue内部的响应式实现监听state以及computed
    后续的vue组件会直接在这上面添加dep
  */
  store._vm = new Vue({
    data: {
      $$state: state
...
/*
初始化module
module加上namespace名字空间（如果有）后，注册mutation、action以及getter，同时递归安装所有子module
*/
